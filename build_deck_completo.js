...
(function () { const sl = p.addSlide(); bgDark(sl); topbar(sl); rodape(sl);
  ...
  ["Dr. Daniel Araujo Fernandes","Dr. Douglas Kosvoski de Ourique","Dr. Marco Galeazi","Dr. Marcos Emilio Kuschnaroff Contreras","Dr. Renan Matineli"].forEach((nm,i)=>{ const cw=9.40,rh=0.72,gap=0.14,x0=(13.33-cw)/2,y0=2.55; const y=y0+i*(rh+gap); sl.addText(nm,{shape:p.ShapeType.roundRect,rectRadius:0.06,fill:{color:CARD},line:{type:"none"},shadow:shadow(),x:x0,y,w:cw,h:rh,color:INK,bold:true,fontSize:19,fontFace:BF,align:"center",valign:"middle",margin:0}); });
})();
// S4 TESE
(function () { const sl = p.addSlide(); bgDark(sl); topbar(sl); rodape(sl);
  sl.addText("ACURÁCIA × ALTERAR O RESULTADO", { x: 0.62, y: 0.40, w: 12.1, h: 0.34, color: TEALB, bold: true, fontSize: 14, charSpacing: 2.5, fontFace: BF, margin: 0 });
  sl.addText("A dissociação entre acurácia e resultado", { x: 0.58, y: 0.76, w: 12.1, h: 1.05, color: INK, bold: true, fontSize: 32, fontFace: HF, valign: "top", margin: 0, lineSpacingMultiple: 1.02 });
...
// S28 SÍNTESE
(function () { const sl = p.addSlide(); bgDark(sl); topbar(sl); rodape(sl);
  sl.addText("ATO 3 · CONCLUSÃO", { x: 0.62, y: 0.40, w: 12.1, h: 0.34, color: TEALB, bold: true, fontSize: 14, charSpacing: 2.5, fontFace: BF, margin: 0 });
